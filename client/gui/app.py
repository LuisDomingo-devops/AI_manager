...
class AeatAutofillWidget(QWidget):
    """Panel de control de Autorelleno del Modelo 303 en la AEAT."""
    def __init__(self, parent_dashboard):
        super().__init__()
        self.dashboard = parent_dashboard
        self.setWindowTitle("ALFONSO AEAT AUTOFILL")
    ...
    def setup_ui(self):
        ...
        container_frame = QFrame()
        container_frame.setObjectName("AeatContainer")
        container_layout = QVBoxLayout(container_frame)
        container_layout.setContentsMargins(20, 20, 20, 20)
        container_layout.setSpacing(15)
        ...
        # Cabecera
        header_layout = QHBoxLayout()
        header_title = QLabel("// ALFONSO OS // AEAT 303 AUTOFILL MODULE")
        header_title.setStyleSheet("font-size: 11px; font-weight: bold; color: #FFB800; letter-spacing: 1px;")
        
        btn_close = QPushButton("[X]")
        btn_close.setFixedWidth(40)
        btn_close.setStyleSheet("background-color: transparent; border: none; color: #FFB800; font-size: 12px; font-weight: bold;")
        btn_close.clicked.connect(self.close)
        ...
        # Grupo de Ingresos
        self.group_income = QGroupBox("INGRESOS DEVENGADOS (IVA REPERCUTIDO)")
        ...
        self.lbl_income_iva.setStyleSheet("color: #00FF66; font-size: 14px; font-weight: bold;")
        income_layout.addRow("Base Imponible [Casilla 01]:", self.lbl_income_base)
        income_layout.addRow("Cuota IVA (21%) [Casilla 03]:", self.lbl_income_iva)
        container_layout.addWidget(self.group_income)

        # Grupo de Gastos
        self.group_expense = QGroupBox("COMPRAS Y GASTOS (IVA DEDUCIBLE)")
        ...
        self.lbl_expense_iva.setStyleSheet("color: #00FF66; font-size: 14px; font-weight: bold;")
        expense_layout.addRow("Base Imponible Corriente [Casilla 28]:", self.lbl_expense_base)
        expense_layout.addRow("Cuota IVA Deducible [Casilla 29]:", self.lbl_expense_iva)
        container_layout.addWidget(self.group_expense)

        # Grupo del Resultado
        self.lbl_result = QLabel("Resultado Neto Estimado: 0.00 €")
        self.lbl_result.setStyleSheet("color: #00E5FF; font-size: 15px; font-weight: bold; padding: 10px; background-color: rgba(0, 229, 255, 0.05); border-radius: 6px; border: 1px solid rgba(0, 229, 255, 0.2);")
        self.lbl_result.setAlignment(Qt.AlignmentFlag.AlignCenter)
        container_layout.addWidget(self.lbl_result)
    ...
    def load_data(self):
        ...
        if res.get("status") == "ok":
            ...
            if quarter_data:
                ...
                net = quarter_data["income"]["total"] - quarter_data["expense"]["total"]
            else:
                self.income_base = 0.0
                self.income_iva = 0.0
                self.expense_base = 0.0
            ...
            self.lbl_result.setText(f"Resultado Neto Estimado: {net:,.2f} €")
        else:
            QMessageBox.warning(self, "Error", f"No se pudieron cargar los datos de impuestos del backend: {res.get('message')}")
