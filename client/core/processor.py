...
class ResponseProcessor:
    ...
    def format_response(result_data: dict) -> str:
        ...
        if result_type == "multi_tool":
            results = result_data.get("results", [])
            messages = []
            for r in results:
                t_name = r.get("tool", "herramienta")
                t_res = r.get("result", {})
                status = t_res.get("status", "")
                if status in ("ok", "success"):
                    msg = t_res.get("message") or f"Listo, ejecuté {t_name} correctamente."
                    messages.append(str(msg))
                else:
                    err = t_res.get("message") or t_res.get("error") or "error"
                    messages.append(f"Error en {t_name}: {err}")
