...
async def test_audit_ledger_tool_integration():
    ...
    with _get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM contacts WHERE name = 'LUIS DOMINGO TEST AUDIT'")
        cursor.execute("INSERT INTO contacts (name, nif, email, contact_type) VALUES ('LUIS DOMINGO TEST AUDIT', '12345678Z', 'luis@example.com', 'Cliente')")
...
def test_get_audit_logs_endpoint():
    ...
    try:
        AuditLedgerService.log_audit_event("API_TEST", "Test api logs", "default")
        ...
        assert data["items"][0]["event_type"] == "API_TEST"
