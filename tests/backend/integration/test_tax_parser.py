...
def mock_llm_generate(monkeypatch):
    async def dummy_generate(self, messages, response_format=None, **kwargs):
        # Infer the response based on the text in messages
        text = str(messages)
        if "F-2026-001" in text:
            return json.dumps({
                "invoice_id": "F-2026-001", "date": "2026-02-15", "issuer_name": "Mi Empresa S.L.",
                "issuer_nif": "12345678Z", "receiver_name": "Cliente Feliz S.A.", "receiver_nif": "B98765432",
                "base_imponible": 1000.0, "iva_amount": 210.0, "total_amount": 1210.0, "type": "ingreso"
            })
        elif "FAC-88992" in text:
            return json.dumps({
                "invoice_id": "FAC-88992", "date": "2026-05-20", "issuer_name": "Proveedor de Internet S.A.U.",
                "issuer_nif": "A55555555", "receiver_name": "Luis Domingo (Autónomo)", "receiver_nif": "12345678Z",
                "base_imponible": 50.0, "iva_amount": 10.5, "total_amount": 60.5, "type": "gasto"
            })
        elif "LUIS DOMINGO" in text or "Retencion IRPF" in text:
            return json.dumps({
                "invoice_id": "LUIS DOMINGO", "date": "2026-11-10", "issuer_name": "Luis Domingo",
                "issuer_nif": "12345678Z", "receiver_name": "Gran Corporacion", "receiver_nif": "B44444444",
                "base_imponible": 2000.0, "iva_amount": 420.0, "irpf_amount": 300.0, "total_amount": 2120.0,
                "type": "ingreso"
            })
        return "{}"
