...
def test_mail_and_subscription_routing_integration(qapp, mock_dashboard_config):
    ...
    with patch("client.gui.app.AlfonsoHUDDashboard.start_agent"), \
        ...
        dashboard.switch_to_view(("espacio_trabajo", "correo_inteligente"))
        assert dashboard.central_stack.currentIndex() == 22

        # Probar navegación a suscripción (ahora índice 30)
        dashboard.switch_to_view(("configuracion", "suscripcion_licencia"))
...
def test_subview_mode_propagation_integration(qapp, mock_dashboard_config):
    ...
    with patch("client.gui.app.AlfonsoHUDDashboard.start_agent"), \
        ...
        dashboard.switch_to_view(("fiscal_contable", "modelos_trimestrales"))
        assert dashboard.central_stack.currentIndex() == 12
        assert dashboard.view_taxes.stack.currentIndex() == 0

        dashboard.switch_to_view(("fiscal_contable", "automatizacion_aeat"))
        ...
        dashboard.switch_to_view(("configuracion", "perfil_fiscal"))
        assert dashboard.central_stack.currentIndex() == 28
...
def test_ledger_navigation_and_mayor_population_integration(qapp, mock_dashboard_config):
    ...
    with patch("client.gui.app.AlfonsoHUDDashboard.start_agent"), \
        ...
        dashboard.switch_to_view(("ingresos_gastos", "facturas_emitidas"))
        assert dashboard.central_stack.currentIndex() == 3
...
def test_help_center_navigation_and_shortcuts_integration(qapp, mock_dashboard_config):
    ...
    with patch("client.gui.app.AlfonsoHUDDashboard.start_agent"), \
        ...
        dashboard.switch_to_view(("configuracion", "centro_ayuda"))
