...
async def test_soft_delete_clients_and_products():
    ...
    r_cli = await create_client(f"Empresa Auditoria {uid}", "12345678Z", f"info_{uid}@auditoria.es")
    ...
    cli = next((c for c in clients_res["clients"] if c["name"] == f"Empresa Auditoria {uid}"), None)
    ...
    cli_active = next((c for c in active_clients["clients"] if c["id"] == cli_id), None)
    assert cli_active is None

    active_prods = await get_products(include_deleted=False)
    prod_active = next((p for p in active_prods["products"] if p["sku"] == f"SKU-AUDIT-{uid}"), None)
    assert prod_active is None

    # 4. Comprobar que siguen existiendo físicamente en BD (Soft Delete)
    with _get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, name, is_active FROM contacts WHERE id = ?", (cli_id,))
        row_c = cursor.fetchone()
        assert row_c is not None
        assert row_c["is_active"] == 0

        cursor.execute("SELECT sku, is_active, deleted_at FROM products WHERE sku = ?", (f"SKU-AUDIT-{uid}",))
