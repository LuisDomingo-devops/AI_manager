...
def test_sidebar_categories_structure_integrity():
    ...
    assert len(SIDEBAR_CATEGORIES) >= 8, "Deben existir al menos 8 categorías principales"
    
    category_ids = set()
    subcategory_ids = set()
    total_subcategories = 0

    # Debe incluir "Dashboard" y "Configuración" como mínimo
    ids = [cat["id"] for cat in SIDEBAR_CATEGORIES]
    assert "dashboard" in ids
    assert "configuracion" in ids
