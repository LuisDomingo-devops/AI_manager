...
def create_email(
    ...
    try:
        cursor = conn.execute(
            ...
            """,
            (
                encryptor.encrypt(sender),
                encryptor.encrypt(recipient),
                encryptor.encrypt(subject),
                encryptor.encrypt(body),
                received_at,
                category,
                importance,
                read_status,
                encryptor.encrypt(summary) if summary else None
            ),
        )
        conn.commit()
...
def get_email(email_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if row:
            d = dict(row)
            d["sender"] = encryptor.decrypt(d["sender"])
            d["recipient"] = encryptor.decrypt(d["recipient"])
            d["subject"] = encryptor.decrypt(d["subject"])
            d["body"] = encryptor.decrypt(d["body"])
            d["summary"] = encryptor.decrypt(d["summary"]) if d["summary"] else None
            return d
    finally:
        conn.close()
    return None
